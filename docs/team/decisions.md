...
3. **Alcance v1**: solo divisiones **horizontales**. Grilla vertical y rebaje por sección quedan fuera.
...
6. **La cantidad de hojas la define el producto, también para las fórmulas** (revisado 2026-07-29): la variable `Cantidad` (alias `hojas`) que reciben todas las fórmulas de despiece salía del *payload* (`configuracion["cantidad_hojas"]`), y **ningún** cotizador la mandaba → se evaluaba siempre con **1**, aunque el producto tuviera 2, 3 o 4 hojas. Ahora, si el payload no la manda, se toma `Producto.cantidad_hojas` (el campo "Cantidad de Hojas"). ⚠️ Esto **sube el precio** de los productos multi-hoja cuyas fórmulas usan la variable, y es un bug **preexistente** que afecta a todo el cotizador, no sólo a los tirantes. Las fórmulas con un **número fijo** (ej. `formula_cantidad = "2"`) **siguen sin escalar**: hay que reescribirlas como `Cantidad * 2`. Diagnóstico para datos reales: `akuna_calc/diagnostico_hojas.py`.
7. **Validación y materiales faltantes en el calculador, no sólo en el serializer** (revisado 2026-07-29): `_validar_secciones` (suma de secciones == alto) vive en `PriceCalculator` porque el precio que se cobra se recalcula desde el guardado del ítem, que **no** pasa por el serializer del API. Y un material inexistente o dado de baja lanza `PricingError` en vez de saltearse: saltear la sección la borraba del precio y cobraba de menos (se midió −37 % en un caso real).
