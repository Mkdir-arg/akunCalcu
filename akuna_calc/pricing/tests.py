...
class CalculatorTerciarizadoTest(SimpleTestCase):
    ...
    def test_terciarizado_usa_precio_manual_del_payload(self):
        from pricing.services.calculator import PriceCalculator

        # El producto NO guarda precio; el precio viene en el payload de cotización.
        producto = SimpleNamespace(terciarizado=True, descripcion='Cortina Roller')
        ...
        with patch.object(PriceCalculator, '_get_marco', return_value=marco):
            result = self._calc().calculate({
                'marco_id': 1, 'ancho_mm': 1000, 'alto_mm': 2000,
                'margen_porcentaje': 0, 'precio_manual_m2': 5000,
            })
    ...
    def test_terciarizado_aplica_margen_del_presupuesto(self):
        ...
        producto = SimpleNamespace(terciarizado=True, descripcion='X')
        marco = SimpleNamespace(id=1, producto=producto)
        with patch.object(PriceCalculator, '_get_marco', return_value=marco):
            result = self._calc().calculate({
                'marco_id': 1, 'ancho_mm': 1000, 'alto_mm': 1000,
                'margen_porcentaje': 50, 'precio_manual_m2': 1000,
            })
    ...
    def test_terciarizado_sin_precio_lanza_error(self):
        from pricing.services.calculator import PriceCalculator, PricingError

        producto = SimpleNamespace(terciarizado=True, descripcion='Sin precio')
        marco = SimpleNamespace(id=1, producto=producto)
        with patch.object(PriceCalculator, '_get_marco', return_value=marco):
