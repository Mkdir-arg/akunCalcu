...
8. **Validación y materiales faltantes en el calculador, no sólo en el serializer** (revisado 2026-07-29): `_validar_secciones` (suma de secciones == alto) vive en `PriceCalculator` porque el precio que se cobra se recalcula desde el guardado del ítem, que **no** pasa por el serializer del API. Y un material inexistente o dado de baja lanza `PricingError` en vez de saltearse: saltear la sección la borraba del precio y cobraba de menos (se midió −37 % en un caso real).
